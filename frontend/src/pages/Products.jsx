...
export function Products({ onOpenQuoteModal }) {
  ...
  const categories = ['All', 'Solar Dryers', 'Hybrid Systems', 'Industrial Solar', 'Industrial Hybrid', 'Custom Systems', 'Dehydrators'];

  const filteredProducts = productList.filter(p => {
    const matchesCat = selectedCategory === 'All' || p.category === selectedCategory;
    const matchesSearch = p.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                          p.shortDescription.toLowerCase().includes(searchQuery.toLowerCase()) ||
                          (p.suitableFor && p.suitableFor.some(s => s.toLowerCase().includes(searchQuery.toLowerCase())));
