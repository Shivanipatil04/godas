...
if (isset($_GET['delete'])) {
    ...
    $pdo->prepare("DELETE FROM gallery_categories WHERE id = :id")->execute(['id' => $delId]);
    header('Location: /admin/gallery-categories.php');
    exit();
}
...
$categories = $pdo->query("SELECT * FROM gallery_categories ORDER BY display_order ASC")->fetchAll();
?>

<div style="display: grid; grid-template-columns: 1fr 2fr; gap: 1.5rem;">
  <div class="card">
    <div class="card-header">
      <h3>Add / Edit Gallery Category</h3>
    </div>
    <div class="card-body">
      <?php if ($message): ?>
      ...
      <form method="POST">
        ...
        <div class="form-group">
          ...
          <input type="text" name="name" id="cat_name" class="form-control" required placeholder="e.g. Installations">
        </div>

        <div class="form-group">
          <label>Slug</label>
          <input type="text" name="slug" id="cat_slug" class="form-control" placeholder="e.g. installations">
        </div>
            ...
            <?php foreach ($categories as $c): ?>
              <tr>
                ...
                <td>
                  ...
                  <a href="/admin/gallery-categories.php?delete=<?= $c['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete category?');">🗑️ Delete</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
